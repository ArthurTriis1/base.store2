import { sendAnalyticsEvent, usePagination, useSearch } from '@faststore/sdk'
...
function ProductGallery({ title, searchTerm }: Props) {
  ...
  const isLoading = data == null || isValidating
  const totalCount = data?.search.products.pageInfo.totalCount ?? 0
  const { next, prev } = usePagination(totalCount)

  /**
   * Send analytics event every time a search occurs.
   * Search engines use this event for populating `topSearches` indices
   * */
  useEffect(() => {
    if (searchState.term !== null && !isLoading) {
      sendAnalyticsEvent<SearchEvent>({
        name: 'search',
        params: {
          search_term: searchState.term,
          records_filtered: totalCount,
          url: href,
        },
      })
    }
  }, [href, isLoading, searchState.term, totalCount])
